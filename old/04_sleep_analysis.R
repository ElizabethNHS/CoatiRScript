...
################# Functions #########################
#rm ( accdata2, d2, IDs, x_d, y_d, z_d, inds)
...
#read in acc data 
acc_dat <- fread( "/Users/capuchin_monkey/Documents/Caps_sleep/DATA/acc_dat.csv" )
...
d1 <- fread( "C:/Users/eliza/Documents/UNI/SS23/MPI-Bachelorarbeit/R_Thesis/Coaticsv/full_night_and_day_data.csv" )
#d1 = d1[-which(d1$tag == "Da Vinci_5764"),]
unique(d1$tag)
...
sum( duplicated( d1[ , c( 'tag', 'local_timestamp' ) ] ) )   #sum is >0 so i have some duplicates.
...
duplicates <- duplicated(d1[, c('tag', 'local_timestamp')])

# Get row numbers of duplicates
duplicate_row_numbers <- which(duplicates)

# Print the row numbers
print(duplicate_row_numbers)

d1[duplicate_row_numbers, ]

head(duplicate_row_numbers)
tail(duplicate_row_numbers)

############## Make a dataframe of sunrise and sunset times ###############
GPSdata=read.csv("C:/Users/eliza/Documents/UNI/SS23/MPI-Bachelorarbeit/R_Thesis/Coaticsv/FFT_Cleaned_Final.csv")  ##Read in GPS data to get location data for sunset and sunrise information 
...
percentile <- 0.425 ## this is the percentile threshold of the variable (within the noon-to-noon time period) used to classify activity vs. inactivity (VeDBA here)
...
sort(unique(d1$local_time))[which(diff(sort(unique(d1$local_time))) != as_hms("00:01:00")) + -1:1]
...
write.csv( sleep_per, '/Users/capuchin_monkey/Documents/Caps_sleep/DATA/sleep_per425_all.csv', row.names = F )
write.csv( full_dat, '/Users/capuchin_monkey/Documents/Caps_sleep/DATA/full_dat425_all.csv', row.names = F )
write.csv( sleep_per_nona, '/Users/capuchin_monkey/Documents/Caps_sleep/DATA/sleep_per_nona425_all.csv', row.names = F )
