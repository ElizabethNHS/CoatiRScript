...
sleep_per_func <- function( tag, night_num, missing_mins = 45, time_gap = 20*60, mov_window = 9, percentile = 0.10, multiplier = 1.125, block_size = 30, gap_size = 45, title = F, x_axis = T, plot_waso = F, waso_window = 1, waso_block = 1, waso_percentile = 0.10, waso_multiplier = 1.125, las = 1, ... ){
  ...
  if(length(time_diffs) != 0){
    ...
    if( nrow( night_dat ) > ( mins_in_day - missing_mins ) & max( time_diffs ) < time_gap ){
      ...
      night_dat$roll_log_vedba <- zoo::rollmedian( night_dat$log_vedba, mov_window, fill = NA, align = 'center' )
      ...
      lines( ts_time, zoo::rollmedian( night_dat$log_vedba, mov_window, fill = NA, align = 'center' ), col = 'red')
      ...
      if( length( which( diffs == 1 ) ) != 0){
        ...
        if( plot_waso == T ){
          ...
          night_dat$SPT_roll_log_vedba <- zoo::rollmedian( night_dat$log_vedba, waso_window, fill = NA, align = 'center' )
...
# <- as.Date( min( d1$local_timestamp )- 12*60*60 )
...
GPSdata=read.csv("C:/Users/salavi/Documents/FFT_Cleaned_Final.csv")  ##Read in GPS data to get location data for sunset and sunrise information 
...
percentile <- 0.50 ## this is the percentile threshold of the variable (within the noon-to-noon time period) used to classify activity vs. inactivity (VeDBA here)
...
for( i in 1:length(tag_names) ){
  ...
  for( j in 1:length(nights) ){
    ...
    #mov_window <- 21 ## this is the size of the moving window (in minutes) used in calculating the rolling median of the average VeDBA ## I had this at 17 due to visual inspection but if we want it to match the validated algorithm, we need to keep it at 9
    #percentile=.5
    #multiplier <- 6 ## this is the multiplier of the threshold value determined by the percentile above. Values below the threshold value multiplied by the multiplier are considered inactive and those equal to or above are considered active
    ...
    dark_start <- sun_dat$night_start[ which(sun_dat$date == as.Date( night, origin = "1970-01-01", tz = "America/Panama" )) ]

    dark_end <- sun_dat$night_end[ which(sun_dat$date == as.Date( night, origin = "1970-01-01", tz = "America/Panama" )) ]
    ...
    first_light <- sun_dat$night_end[ which(sun_dat$date == ( as.Date( night, origin = "1970-01-01", tz = "America/Panama" )  - 1 )) ]
...
sleep_per <- data.frame( tag = rep( unique( d1$tag ), each = length( study_nights ) ), night = rep( study_nights, times = length( tag ) ), onset = NA, waking = NA, SPT = NA, WASO = NA, TST = NA, TST_in_median_SPT = NA, TST_at_sleep_site = NA, sleep_eff = NA, wake_bouts = NA, frag_wake_bouts = NA, frag_wake_bouts_in_median_SPT = NA, frag_wake_bouts_at_sleep_site = NA, summed_VeDBA = NA, night_VeDBA_corr = NA, ave_vedba = NA, dark_TST = NA, light_TST = NA, dark_sleep_eff = NA, light_sleep_eff = NA, prev_naps = NA, prev_day_sleep = NA, prev_day_sleep_lim = NA, prev_day_ave_vedba = NA, poss_dark_bursts = NA, n_dark_bursts = NA, poss_day_bursts = NA, n_day_bursts = NA, max_time_diff = NA, n_bursts= NA, max_dark_time_diff = NA, max_day_time_diff = NA, SPT_poly = NA, TST_poly = NA )
...
for( i in 1:length(tag_names) ){
  ...
  for( j in 1:length(nights) ){
    ...
    if( nrow( SPT_dat ) > 0 ){
      ...
      waking_dat <- id_dat[ which(id_dat$local_timestamp > sleep_per[ sleep_per$tag == tag_names[i] & sleep_per$night == ( as.Date( night, origin = "1970-01-01", tz = "America/Panama" ) - 1 ), ]$waking & id_dat$local_timestamp <  onset   ), ]
...
checkdates=data.frame(unique(cbind(acc_dat$tag, as.character(acc_dat$night))))
...
#sleep_per[ ( sleep_per$n_dark_bursts < ( sleep_per$poss_dark_bursts - night_missing_mins ) | sleep_per$max_dark_time_diff > time_gap ) & !is.na( sleep_per$poss_dark_bursts ), c( 'dark_TST', 'dark_sleep_eff', 'light_TST', 'light_sleep_eff', 'ave_vedba' ) ] <- NA

#sleep_per[ ( sleep_per$n_day_bursts < ( sleep_per$poss_day_bursts - missing_mins ) | sleep_per$max_day_time_diff > time_gap ) & !is.na( sleep_per$poss_day_bursts ), c( 'prev_day_sleep', 'prev_day_ave_vedba', 'prev_day_sleep_lim' ) ] <- NA
...
#hist( sleep_per$TST_in_median_SPT )
