...
accdata2 = read.csv("/Users/wilddog/Documents/eneuhaus/Coaticsv/accdata2.csv") #in the case that accdata2  isnt in the global environment
...
#looking for NAs before L45
#sum(is.na(accdata2$timestamp))
...
class(complete_data$timestamp)

indexes = which(is.na(complete_data$timestamp) == TRUE)
complete_data = accdata2
complete_data$timestamp[indexes] = paste(complete_data$timestamp[indexes], "00:00:00", sep = " ") # !!!!lost info abt time when data was loaded. we forceibly added that info to avoid NAs when converting to POSIX by changing it while still in character format. if theres a date w no info its midnight.
complete_data$timestamp <- as.POSIXct(x= complete_data$timestamp, format=c("%Y-%m-%d %H:%M:%S"), tz='UTC' )
head(complete_data$timestamp[indexes])
...
class(complete_data$local_timestamp)
...
tag_names <- as.character( unique( acc_dat$individual_local_identifier ) ) #include animal name and identifier. "as.character()" converts unique tag values into character format.
...
# adding names from complete data dataframe to acc_dat
acc_dat$individual_local_identifier <- complete_data$individual_local_identifier

# create folder on computer to save vedba data in 
write.csv(acc_dat, "/Users/wilddog/Documents/eneuhaus/Coaticsv/full_night_and_day_data.csv", row.names = F) #saves the vedba 


head( acc_dat )


######unsuccessful attempts at removing POSIXt elements of the local timestamp column of d1 or converting them to POSIXct. not possible because the column contains dates (POSIXct) and times (POSIXt).

#class(d1$local_timestamp)
...
# Find elements of class POSIXt in the column
#posixt_indices <- which(class(d1$local_timestamp) == "POSIXt")

# Remove elements of class POSIXt from the column
#d1$local_timestamp <- d1$local_timestamp[-posixt_indices]

#d1$local_timestamp <- as.POSIXct(d1$local_timestamp)
#class(d1$local_timestamp)

#posixt_indices <- which(sapply(d1$local_timestamp, is.POSIXt))
#d1$local_timestamp <- d1$local_timestamp[!posixt_indices]


#length(d1$local_timestamp)
#length(posixt_indices)
